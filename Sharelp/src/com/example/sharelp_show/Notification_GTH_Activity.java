...
/**
 * 直接从服务器的文件夹获取图片
 * 从outputstream/output2获取数据
 * @author Administrator
 *
 */


public class Notification_GTH_Activity extends Activity implements OnItemClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initView();
		initHandler();
		Util_ReadEntity.OLink(handler, Entity_Gth[].class,Util_Const.GTH,10000);
		initListen();
	}
		...
		handler = new Handler(){
			public void handleMessage(Message msg) {
				if (msg.arg1==200) {
					...
					listView.setAdapter(new Entity_GthAdapter(Notification_GTH_Activity.this,(Entity_Gth[])msg.obj, 
							R.layout.activity_listview_item_gth, cache));
				}else {
					Toast.makeText(Notification_GTH_Activity.this, "连接服务端失败", 2000).show();
				}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long arg3) {
		...
		Entity_Gth entity_Gth=(Entity_Gth) parent.getItemAtPosition(position);
		  ...
		  Intent mIntent = new Intent(Notification_GTH_Activity.this,Inform_DetailsActivity.class);     
	...
	/*	Notification_Gth_DetailsActivity.NAME=entity_Gth.getName();
		Notification_Gth_DetailsActivity.AUTHOR=entity_Gth.getAuthor();
		Notification_Gth_DetailsActivity.PATH=entity_Gth.getPath();
		Notification_Gth_DetailsActivity.PRIZE=entity_Gth.getPrize();
		Notification_Gth_DetailsActivity.INTRODUCTION=entity_Gth.getIntroduction();

		Intent intent=new Intent(Notification_GTH_Activity.this, Notification_Gth_DetailsActivity.class);
